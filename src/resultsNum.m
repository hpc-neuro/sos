function  [soa_ndx score ind] = resultsNum(flag, soa)
    
    global exp 
    
    soa = [20 40 80 160];
   
    %path_r = '/Users/sb/neuro_comp/tmp/';
    path_r = '/Users/petavision/neuro_comp/exp/vadasg/data/';
    mask_lab = dir([path_r 'masks/*.png']);
    ...
    score = struct( ...
        ...
        'sn', zeros(length(soa), length(masks), num, dist));
    ...
    soa_ndx = cell(4,1);
    sn_ndx = cell(length(soa), length(masks), num, dist);
    ...
    for i = 1:rows
        ...
        if ischar(left(i).name)
            ...
            %ind(i) = ndx;
            if isempty(str2num(left(ndx(i)).name(12)))
                num_label(i) = str2num(right(ndx(i)).name(10));
                dist_label{i} = left(ndx(i)).name(12);
            else
                num_label(i) = str2num(left(ndx(i)).name(12));
                dist_label{i} = right(ndx(i)).name(10);
            end
            ...
            soa_char = left(ndx(i)).name(10);
            mask_char{i} = mask_lab(ndx(i)).name(10);
...
%             end
        ...
        for i = 1:length(soa)
            for j = 1:length(masks)
                for k = 1:num
                    for ii = 1:dist
                        sn_ndx{i,j,k,ii} = r(soa_label == i & strcmp(mask_char, masks{j}) & ...
                            num_label == S2(k) & strcmp(dist_label,S{ii}));
                        score.sn(i,j,k,ii) = sum(sn_ndx{i,j,k,ii} == 0) / (sum(sn_ndx{i,j,k,ii} ...
                            == 0) + ...
                            sum(sn_ndx{i,j,k,ii} == ...
                            1) + ...
                            sum(sn_ndx{i,j,k,ii} == ...
